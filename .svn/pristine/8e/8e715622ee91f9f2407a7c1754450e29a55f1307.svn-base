...
//    return;
    ...
    [request setFailedBlock:^{
        ...
        if (completion)
        {
            NSString *errorDomain = _requestCopy.error.domain;
            if ([errorDomain isEqualToString:@"ASIHTTPRequestErrorDomain"] || [errorDomain isEqualToString:@"NSCocoaErrorDomain"])
            {
                errorDomain = MyLocal(@"网络不可用，请检查网络设置");
            }
            
            NSError *error = [NSError errorWithDomain:errorDomain code:_requestCopy.error.code userInfo:nil];
